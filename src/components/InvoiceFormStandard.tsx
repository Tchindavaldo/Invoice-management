        ...
        {formData.items.length === 0 && (
          <div className={`text-center py-8 rounded-lg border-2 border-dashed transition-colors ${
            ...
            <p className={`${errors.items ? 'text-red-600 font-semibold' : 'text-gray-500'}`}>
              {errors.items || 'Aucun article ajouté. Cliquez sur "Ajouter une ligne" pour commencer.'}
            </p>
            {errors.items && (
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => handleInputChange('customsFees', Number(e.target.value))}
              min="0"
              step="0.01"
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-600"
              placeholder="0.00"
            />
            <p className="text-xs text-gray-500 mt-1">Montant des frais de dédouanement à ajouter au total</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Devise
